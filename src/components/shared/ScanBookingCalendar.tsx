...
export interface ScanBookingUserInfo {
  fullName: string;
  email: string;
  phone: string;
}
...
export function ScanBookingCalendar({ userInfo, bookingType, calendarId, onBookingComplete, onCancel }: ScanBookingCalendarProps) {
  ...
  const { bookedSlots24h, isLoading: slotsLoading } = useBookedSlots(selectedDate ?? null, calendarId);
  const { fullyBookedDates } = useFullyBookedDates(visibleMonth, calendarId);
  ...
  return (
    <div className="border border-lioner-gold/30 rounded-lg p-4 bg-lioner-gold/5 space-y-4">
      <h4 className="text-sm font-bold text-foreground uppercase tracking-wider">
        {language === "nl" ? "Plan een sessie" : "Schedule a Session"}
      </h4>
      ...
      <div className="flex gap-2">
        <Button variant="ghost" size="sm" onClick={onCancel} className="text-foreground/50">
